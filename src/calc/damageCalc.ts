import type { TeamSlot, PokeEntry, StatMap, AdvOverride, TableRow } from '../types'
...
function isGrounded(defData: PokeEntry): boolean {
  if (defData.t1 === 'Flying' || defData.t2 === 'Flying') return false
  if (defData.ab === 'Levitate') return false
  return true
}
...
export function calcBestMove(
  ...
  const defIsGrounded = isGrounded(defenderData)
  ...
  for (let mi = 0; mi < selectedMoves.length; mi++) {
    const moveName = selectedMoves[mi]
    const md = MOVE_DATA[moveName]
    if (!md || !md.bp || md.bp <= 0) continue
    if (!md.category || md.category === 'Status') continue
    ...
    if (!bestResult || maxPct > bestResult.maxPct) {
      bestResult = { moveName, moveType, moveCat, minPct, maxPct, typeEff, defHP, minDmg, maxDmg }
    }
  }
...
export function buildTableRow(
  ...
  const result = calcBestMove(attacker, atkStats, defName, defData, override, weather, terrain)
  ...
  return {
    ...
    type1: defData.t1,
    type2: defData.t2 || '',
    usage: 0, // filled by caller from USAGE_MAP
    move: result.moveName,
    moveType: result.moveType,
    moveCategory: result.moveCat,
    minPct: result.minPct,
    maxPct: result.maxPct,
    minDmg: result.minDmg,
    maxDmg: result.maxDmg,
    defHP: result.defHP,
    isOHKO: result.minPct >= 100,
    isKO: result.maxPct >= 100 && result.minPct < 100,
  }
}
